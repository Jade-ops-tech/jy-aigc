...
CodeBuild 使用 Amazon Linux 2023 AArch64 镜像，并在 buildspec 中显式选择
Node.js 22，以匹配仓库的 pnpm 11 工具链。
...
Cloud Map 私有 DNS namespace 会自动创建对应的 Route 53 private hosted zone，因此
第三层角色还包含创建、读取和删除该 hosted zone 所需的最小 Route 53 权限。

Webhook 还必须匹配 GitHub 数字账号 ID。这样来自陌生账号或 fork 的 PR不能运行
拥有部署能力的构建。不要移除该过滤条件。
...
### 5. 配置手动 CodeBuild webhook

模板有意不让 CodeBuild GitHub App 自动修改仓库 webhook。流水线创建成功后运行：

```bash
bash scripts/configure-pr-webhook.sh
```

脚本调用 CodeBuild 的 manual webhook API 取得 Payload URL 和随机 Secret，并通过
当前 `gh` 登录把它写入 GitHub。Secret 只在脚本进程内传递，不会打印或落盘。
Webhook 只接受目标为 `main`、并由 GitHub 数字账号 `148870035` 触发的 PR 事件。
